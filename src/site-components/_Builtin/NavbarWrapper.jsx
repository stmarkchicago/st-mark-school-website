...
import { EASING_FUNCTIONS, KEY_CODES, cj, debounce, extractElement, isServer, useLayoutEffect, useResizeObserver, } from '../utils';
...
function NavbarOverlay({ children }) {
    ...
    return (React.createElement("div", { className: "w-nav-overlay", id: "w-nav-overlay", style: {
            ...
            width: isOpen ? '100%' : 0,
        }, onClick: overlayToggleOpen, onKeyDown: overlayToggleOpen }, children));
}
function Navbar({ tag = 'div', className = '', children, config, ...props }) {
    ...
    const { childMenu, rest } = React.useMemo(() => maybeExtractChildMenu(children, shouldExtractMenu), [children, shouldExtractMenu]);
    const handleFocus = (e) => {
